/**
 * React wrapper for the standalone OnboardingNumbers step.
...
import type { OnboardingNumbersStep } from '../components/account-onboarding/step-numbers';
import type { DialStackInstanceImpl } from '../types/core';

// Ensure the custom element is registered
import '../components/account-onboarding/step-numbers';

export interface OnboardingNumbersProps {
  className?: string;
...
export const OnboardingNumbers: React.FC<OnboardingNumbersProps> = ({
  className,
  style,
  locale,
  formatting,
  ...
  const componentRef = useRef<OnboardingNumbersStep | null>(null);
  const [instance, setInstance] = useState<OnboardingNumbersStep | null>(null);
  ...
  const containerRef = useCallback(
    (container: HTMLDivElement | null) => {
      ...
      const el = document.createElement('dialstack-onboarding-numbers') as OnboardingNumbersStep;
