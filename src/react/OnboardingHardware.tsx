/**
 * React wrapper for the standalone OnboardingHardware step.
...
import type { OnboardingHardwareStep } from '../components/account-onboarding/step-hardware';
import type { DialStackInstanceImpl } from '../types/core';

// Ensure the custom element is registered
import '../components/account-onboarding/step-hardware';

export interface OnboardingHardwareProps {
  className?: string;
...
export const OnboardingHardware: React.FC<OnboardingHardwareProps> = ({
  className,
  style,
  locale,
  formatting,
  ...
  const componentRef = useRef<OnboardingHardwareStep | null>(null);
  const [instance, setInstance] = useState<OnboardingHardwareStep | null>(null);
  ...
  const containerRef = useCallback(
    (container: HTMLDivElement | null) => {
      ...
      const el = document.createElement('dialstack-onboarding-hardware') as OnboardingHardwareStep;
